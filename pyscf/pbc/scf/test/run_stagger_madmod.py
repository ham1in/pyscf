...
Nk = np.prod(kmesh)
# Verify and check that files exist
from pathlib import Path
chk_dir = '/home/sjquiton/pyscf-files/singularity_subtraction/sqg-chk-storage/'
df_dir = '/home/sjquiton/pyscf-files/singularity_subtraction/df-storage/from_lawrencium/'
chk_file = chk_dir+'phosphorous-kmf-nk444.chk'
df_file = df_dir+'phosphorous-df-nk444.h5'
sqg_file = chk_dir+'phosphorous_SqG_nk444.npy'
_ = Path(df_file).resolve(strict=True)
_ = Path(chk_file).resolve(strict=True)
_ = Path(sqg_file).resolve(strict=True)


# Read SCF Result and make KMF 
from pyscf.lib import chkfile
chkfile_result =chkfile.load(chk_file,'scf')
mf = khf.KRHF(cell, exxdiv='ewald')
